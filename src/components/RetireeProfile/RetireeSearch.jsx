            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
              ...
              {filteredRetirees.length === 0 ? (
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p>{t("retiree.search.noResults", "No retirees found matching your criteria.")}</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {filteredRetirees.map(r => {
                    ...
                    const highlightedInterests = retireeInterests.map((interest, index) => {
                      ...
                      return (
                        <span key={index} className={`inline-block mr-2 mb-1 px-2 py-1 rounded-full text-xs ${
                          isMatch 
                    ...
                    return (
                      <div key={r.id} className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                        <div className="flex items-start justify-between mb-3">
                          <div>
                            ...
                            <div className="text-sm text-gray-600 mt-1">
                              {t("retiree.search.age", "Age")}: {r.idVerification?.age || t("common.notAvailable", "N/A")} • {t("retiree.search.gender", "Gender")}: {r.idVerification?.gender || t("common.notAvailable", "N/A")}
                            </div>
                          </div>
                        </div>
                        
                        <div className="space-y-2 mb-4">
                          <div className={`text-sm ${isJobMatch ? "font-medium" : ""}`}>
                          ...
                          <div className="text-sm">
                            ...
                            <div className="flex flex-wrap">
                              {highlightedInterests.length > 0 ? highlightedInterests : (
                                <span className="text-gray-500 text-xs">{t("common.notAvailable", "N/A")}</span>
                              )}
                            </div>
                          </div>
                        </div>
                        ...
                        <button
                          className="w-full bg-blue-500 hover:bg-blue-600 active:bg-blue-700 text-white font-medium py-3 px-4 rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
